...
pub const Socket = struct {
    ...
    pub fn set(socket: Socket, comptime option: Option, value: OptionType(option)) SetOptionError!void {
        const raw_value = if (OptionType(option) == bool)
            @as(c_int, @intFromBool(value))
        else
            value;
        const RawValue = @TypeOf(raw_value);

        const ptr, const size = if (RawValue == []const u8 or
            RawValue == [:0]const u8)
            .{ raw_value.ptr, raw_value.len }
        else if (RawValue == c_int or
            RawValue == u64 or
            RawValue == i64 or
            RawValue == bool)
            .{ &raw_value, @sizeOf(RawValue) }
        else {
            @compileError("Unrecognized type: " ++ @typeName(RawValue));
        };
